  ...
  return (
    <nav className="shadow-md bg-linear-to-br from-blue-700 to-sky-700 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-5 py-4 flex items-center gap-4">
            ...
            <Link to="/" className="text-4xl text-black font-bold justify-self-start grow">
                Comanda Bravo
            </Link>
            ...
            <div className="hidden md:flex space-x-6">
                ...
                : user?.role === 20 ?
                <NavLink to = "/admin" className={' px-4 py-3 rounded-lg font-medium text-white transition duration-300 hover:bg-sky-200 hover:text-blue-500'}>Панель ветеринара</NavLink>
                :
                <NavLink to = "/profile" className={' px-4 py-3 rounded-lg font-medium text-white transition duration-300 hover:bg-sky-200 hover:text-blue-500'}>{user?.username}</NavLink>
                }
            </div>
        </div>
    </nav>
